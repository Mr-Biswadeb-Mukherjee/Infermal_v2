...
import (
	...
	config "github.com/Mr-Biswadeb-Mukherjee/Infermal_v2/Engine/Modules/app/core/config"
	cooldown "github.com/Mr-Biswadeb-Mukherjee/Infermal_v2/Engine/Modules/app/core/cooldown"
	logger "github.com/Mr-Biswadeb-Mukherjee/Infermal_v2/Engine/Modules/app/core/logger"
	ratelimiter "github.com/Mr-Biswadeb-Mukherjee/Infermal_v2/Engine/Modules/app/core/ratelimiter"
	redis "github.com/Mr-Biswadeb-Mukherjee/Infermal_v2/Engine/Modules/app/core/redis"
	ui "github.com/Mr-Biswadeb-Mukherjee/Infermal_v2/Engine/Modules/app/core/ui"
	wpkg "github.com/Mr-Biswadeb-Mukherjee/Infermal_v2/Engine/Modules/app/core/worker"
)
...
func Run(parentCtx context.Context) error {
	...
	appLog := logger.NewLoggerInDir("app", logsDir())
	dnsLog := logger.NewLoggerInDir("dns", logsDir())
	rateLimiterLog := logger.NewLoggerInDir("ratelimiter", logsDir())
	defer appLog.Close()
	defer dnsLog.Close()
	...
	// Load config
	cfg, err := config.LoadOrCreateConfig(settingFile("setting.conf"))
	if err != nil {
		close(animStop)
	...
	// Redis init
	if err := redis.Init(settingFile("redis.yaml")); err != nil {
		close(animStop)
	...
	// Domain generation
	scoredGenerated, err := recon.GenerateScoredDomains(inputFile("Keywords.csv"))
	if err != nil {
		close(animStop)
